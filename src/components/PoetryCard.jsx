...
export default function PoetryCard() {
    ...
    const loadMoreItems = () => {
        if (loading || visibleItems >= sortedPoetry.length) return;
        setLoading(true);
        setTimeout(() => {
            setVisibleItems((prev) => Math.min(prev + 10, sortedPoetry.length));
            setLoading(false);
    ...
    return sortedPoetry.slice(0, visibleItems).map((item, id) => {
        return (
